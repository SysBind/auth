    ...
    if ($rs === false) {
        ...
        foreach ($saml_roles as $role) {
            ...
            if (empty($role['lms_role']) || empty($role['saml_role'])) {
                $err['role_mapping_db'][] = "<p>" . get_string("auth_saml_error_attribute_role_mapping", "auth_saml")
                    . "</p><p>saml_role: " . $role['saml_role'] . " lms_role: " . $role['lms_role'] . "</p>";
            } else {
...
function print_role_mapping_options($role_mapping, $config, $err) {
    ...
    foreach ($role_mapping as $saml_role => $mrole) {
            ...
            foreach ($moodle_roles as $mrole2) {
                echo '<option value="' . $mrole2 . '" ' . ((!$read_from_db_only && $role_param[0] == $mrole2)
                    || ($read_from_db_only && $mrole2 == $mrole) ? 'selected="selected"' : '') .' >' . $mrole2 . '</option>';
            }
            echo '</select></td>';
    ...
    if ($read_from_db_only) {
        ...
        while ($i <= $new_roles_total) {
	        ...
	        echo '<tr ' . (empty($new_role_param[1]) ? 'style="display:none;"' : ((isset($err['role_mapping']['lms']) && in_array($new_role_param[0], $err['role_mapping']['lms']))
	            || (isset($err['role_mapping']['saml']) && in_array($new_role_param[1], $err['role_mapping']['saml'])) ? 'style="background:red;"' : '')) .'>';
